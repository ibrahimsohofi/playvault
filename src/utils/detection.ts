    ...
    try {
      ...
      pc.onicecandidate = (e) => {
        if (!e.candidate) {
          ...
          const match = /([0-9]{1,3}(\.[0-9]{1,3}){3})/.exec(e.candidate.candidate);
          if (match && match[1]) addresses.push(match[1]);
        }
      };
...
const checkAdBlockerExtensions = (): boolean => {
  ...
  return selectors.some(selector => document.querySelector(selector)) ||
    ['adblockDetect', 'blockAdBlock', 'fuckAdBlock', 'abp'].some(key => (window as any)[key] !== undefined);
};

// Custom hook to use VPN detection
export const useVPNDetection = (): { isVPN: boolean, confidence: number, reason?: string } => {
  const [result, setResult] = useState({ isVPN: false, confidence: 0, reason: '' });
