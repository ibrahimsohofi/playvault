...
import { Filter, ChevronDown, ChevronUp, Star, Download, ShieldAlert, Heart, Gamepad, AlertTriangle } from "lucide-react";
...
function VPNBlockDialog({ open }: { open: boolean }) {
  return (
    <Dialog open={open}>
      <DialogContent className="max-w-xs sm:max-w-md text-center p-8 border-2 border-red-500/40 shadow-xl animate-in fade-in slide-in-from-top-4">
        <div className="flex flex-col items-center mb-2">
          <AlertTriangle className="h-12 w-12 text-red-500 mb-3 animate-bounce" />
          <DialogTitle className="text-2xl font-bold mb-2 text-red-500">VPN/Proxy Detected</DialogTitle>
          <p className="text-base text-muted-foreground mb-2">
            Please turn off your VPN or proxy (or try another browser) to continue your download.
          </p>
        </div>
      </DialogContent>
    </Dialog>
...
async function checkVPNProxy(): Promise<boolean> {
  try {
    const response = await fetch('https://ipapi.co/json/');
    const data = await response.json();
    return Boolean((data.security && (data.security.vpn || data.security.proxy || data.security.tor)));
  } catch {
    // On network failure, do not block the user
    return false;
  }
}
...
export function ResourceLibrary() {
  ...
  const handleDownloadVerifyClick = async (resource: GameResource) => {
    // Check for VPN/proxy before proceeding
    setVpnBlocked(false);
    const vpnDetected = await checkVPNProxy();
    if (vpnDetected) {
      setVpnBlocked(true);
      return;
    }
  ...
  return (
    <section id="games" className="py-20 relative">
      <div className="container-custom">
        ...
        {/* VPN/Proxy Block Dialog */}
        <VPNBlockDialog open={vpnBlocked} />
