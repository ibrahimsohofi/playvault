...
import { DynamicAdBlueMediaLocker } from "../shared/DynamicAdBlueMediaLocker";
import { VerificationDialog } from "../shared/VerificationDialog";
import { detectAdBlocker, detectVPN } from "@/utils/detection";
...
export function ResourceLibrary({ initialCategory = null }: ResourceLibraryProps) {
  ...
  const handleDownloadVerifyClick = async (resource: GameResource) => {
    ...
    try {
      // Check for VPN first with priority 0
      const vpnResult = await detectVPN();
      console.log('VPN detection result:', vpnResult);

      if (vpnResult.isVPN) {
        console.log('VPN detected during download - showing dialog');
        queueDialog({
          type: 'vpn',
          props: {
      ...
      // Check for ad blocker with priority 1
      const adBlockerResult = await detectAdBlocker();
      console.log('Ad blocker check result:', adBlockerResult);

      if (adBlockerResult) {
        console.log('Ad blocker detected during download - showing dialog');
        queueDialog({
          type: 'adblock',
          props: {
            isDownload: true,
      ...
      // If no VPN or AdBlocker detected, proceed with verification
      console.log('No blocking issues detected - proceeding with verification');
      setPendingResource(resource);
      setVerifying(true);
    } catch (error) {
      console.error('Error during download verification:', error);
    }
  };
    ...
    >
        ...
        <CardContent className="p-6">
          ...
          <div className="flex gap-2">
            <Link to={`/games/${resource.id}`} className="flex-1">
              <Button
                variant="outline"
                className="w-full border-[#00f7ff]/30 hover:bg-[#00f7ff]/10"
                onClick={(e) => {
                  e.stopPropagation();
                }}
              >
