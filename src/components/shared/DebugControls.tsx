...
import { Unlock, Settings, Bug, TestTube, Wrench } from 'lucide-react';
...
export function DebugControls({ className }: DebugControlsProps) {
  ...
  const toggleTestingMode = () => {
    const isTestingMode = localStorage.getItem('adblue_testing_mode') === 'true';
    localStorage.setItem('adblue_testing_mode', isTestingMode ? 'false' : 'true');
    window.location.reload();
  };

  const bypassAdBlocker = () => {
    localStorage.setItem('bypass_adblocker', 'true');
    window.location.reload();
  };
  ...
  const testAdBlueFunction = () => {
    // Open debug page in new tab
    window.open('/debug-adblue.html', '_blank');
  };

  const isTestingMode = localStorage.getItem('adblue_testing_mode') === 'true';
  const adBlockerBypassed = localStorage.getItem('bypass_adblocker') === 'true';
        ...
        <div className="bg-black/90 border border-cyan-500/30 rounded-lg p-4 min-w-[300px]">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {isTestingMode ? 'Disable' : 'Enable'} Testing Mode
            </Button>

            <Button
              size="sm"
              variant="outline"
              onClick={bypassAdBlocker}
              className="w-full text-xs"
            >
