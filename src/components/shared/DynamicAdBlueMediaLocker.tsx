...
import { getAdBlueMediaConfig, type AdBlueMediaConfig } from "@/data/lockerConfig";
...
export function DynamicAdBlueMediaLocker({
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        try {
          ...
          console.log('🔧 Setting up global variable BEFORE script load:', config.variable, {
            it: config.it,
            key: config.key
          });

          // Set up the global variable directly instead of using a script tag
          ...
          console.error('Error creating AdBlueMedia config:', error);
          reject(new Error(`Failed to create config: ${error instanceof Error ? error.message : 'Unknown error'}`));
          return;
        }
  ...
  useEffect(() => {
    ...
    const initializeLocker = async () => {
      ...
      try {
        ...
        setIsLoading(false);
        console.log(`✅ AdBlueMedia locker constructed successfully for ${gameId}`);
      } catch (error) {
  ...
  }, [isOpen, contentUnlocked, gameId, isLoading, onClose, loadAdBlueMediaScripts]);
